...
<div class="row">
    <div class="columns large-6">
        ...
        <?php
        try {
			$dsn = "mysql://app:mypass@localhost/my_app";
			ConnectionManager::config('my_connection', ['url' => $dsn]);
            $connection = ConnectionManager::get('my_connection');
            $connected = $connection->connect();
			#$connection->query("CREATE USER 'app'@'localhost' IDENTIFIED BY 'mypass';",$connection);
			#$connection->query("GRANT ALL ON my_app.* TO 'app'@'localhost'");
